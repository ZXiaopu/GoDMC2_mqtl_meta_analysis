...
class Mapper(object):
    ...
    # @timing
    def fill(self, keys, name, path=None, repeats=False, reference=None):  # TODO (middle) remove
        """
		Fill the mapper
    ...
    def load_flip(self, folder, encode=None):
        ...
        for j, i in enumerate(self.genotype_names):
            if encode is not None:
                ...
                if encode[j] == 0:
                    self.flip[i] = np.load(os.path.join(folder, 'flip_' + self.reference_name + '_' + i + '.npy'), allow_pickle=True)
                elif encode[j] == 1:
                    flip = np.load(os.path.join(folder, 'flip_' + self.reference_name + '_' + i + '.npy'), allow_pickle=True)
    ...
    def load(self, folder):
        ...
        for j, i in enumerate(self.genotype_names):
            self.values[:, j] = np.load(os.path.join(folder, 'values_' + self.reference_name + '_' + i + '.npy'), allow_pickle=True)
        ...
        if self.include is not None or self.exclude is not None or True:
            self.reference = Reference(self.reference_name, self.reference_path)
    ...
    # @timing
    def get(self, chunk_number=None, allowed_missingness_rate=0, chromosomes=False):
        ...
        if self.processed == self.n_keys:
            return (None, None, None) if chromosomes else (None, None)
            ...
            if not self.is_filtered:
                ...
                if self.include is not None:
                    if 'ID' in self.include.columns:
                        ...
                        print("Number of variants that remain after excluding variants not in mapper: {}".format(
                            self.include.shape[0]))
                    elif 'CHR' in self.include.columns and 'bp' in self.include.columns:
        ...
        if allowed_missingness_rate == 1:
            ...
            indices_for_values_to_analyze = np.where(
                (1 - (self.values[:, :] > -1).mean(axis=1)) <= allowed_missingness_rate)[0]

        indices_for_values_to_analyze = indices_for_values_to_analyze[
            indices_for_values_to_analyze >= start]

        # Loop over chunks, popping them, while the current chunk does not contain more than self.chunk_size values.

        while np.sum(indices_for_values_to_analyze < finish) < self.chunk_size:
            if self.processed == self.n_keys:
                break
        ...
        if np.sum(indices_for_values_to_analyze < finish) > 1.5 * self.chunk_size:
            if chunk_number is None:
        ...
        indices_for_values_with_all_studies_available = indices_for_values_to_analyze[
            indices_for_values_to_analyze < finish]
        ...
        if chromosomes:
            raise NotImplementedError()
            #chromosome_array = self.reference.chromosome_array[indices_for_values_with_all_studies_available]
            #return [indexes[:, i].astype(np.int64) for i in range(self.n_study)], indices_for_values_with_all_studies_available, chromosome_array.values

        return [indexes[:, i].astype(np.int64) for i in range(self.n_study)], indices_for_values_with_all_studies_available
            ...
            if self.path.get(self.name) is not None:
                try:
                    self.dataframe = pd.read_csv(self.path[self.name]['table'], compression='gzip', sep=' ',
                                                 chunksize=self.chunk, dtype={'ID': 'str'})
                except:
                    self.dataframe = pd.read_csv(self.path[self.name]['table'], sep=' ', chunksize=self.chunk, dtype={'ID': 'str'})
                self.loaded = True
            else:
                if os.path.isfile(os.path.join(self.path_default, self.name)):
                    try:
                        self.dataframe = pd.read_csv(os.path.join(self.path_default, self.name), compression='gzip',
                                                     sep=' ', chunksize=self.chunk, dtype={'ID': 'str'})
                    except:
