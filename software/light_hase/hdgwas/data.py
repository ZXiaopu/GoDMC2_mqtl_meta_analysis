...
class MetaPhenotype(object):
    ...
    def __init__(self, phen, protocol=None, include=None, exclude=None,
        ...
        # @timing
        def _check(map, keys, max_missingness_rate=0):
            values = np.array(map.dic.values())
            result = {}
            if max_missingness_rate == 0:
                r = (values == -1).any(axis=1)
            elif max_missingness_rate == 1:
                r = (values == -1).all(axis=1)
            else:
                r = (values == -1).mean(axis=1) > max_missingness_rate
        ...
        if protocol is None:
            ...
            self.order, self.phen_names = _check(self.mapper, self.keys, max_missingness_rate=allowed_missingness_rate)
...
class MetaParData(object):
    ...
    def __init__(self, pd, study_names, protocol=None, allowed_missingness_rate=0, match_covs=True):
        # @timing
        def _check(map, keys, max_missingness_rate=0):
            values = np.array(map.dic.values())
            result = {}
        ...
        if protocol is None:
            for i, k in enumerate(pd):
                ...
                if i == 0:
                    ...
                    self.phen_mapper.push(k.folder._data.metadata['phenotype'], name=k.folder.name, new_id=allowed_missingness_rate > 0)
                    self.cov_mapper.push(self.covariates[k], name=k.folder.name, new_id=allowed_missingness_rate > 0)
                keys.append(k.folder.name)

            self.phen_order, self.phen_names = _check(self.phen_mapper, keys, max_missingness_rate=allowed_missingness_rate)
            if match_covs:
                self.cov_order, self.cov_names = _check(self.cov_mapper, keys)
            else:
                self.cov_order = {
    ...
    def get_covariate_names(self, study_name):
        return self.covariates[self.pd[study_name]]
...
class NPFolder(Folder):
    ...
    def __init__(self, path, protected=[]):
        ...
        try:
            ...
            self.data_info = np.load(os.path.join(file_path, 'info_dic.npy'), allow_pickle=True).item()
    ...
    def read(self, file):
        ...
        # print 'end cache'
            ...
            # print 'reading file {}'.format(file)
            d = np.load(os.path.join(self.path, file), allow_pickle=True)
...
class PDFolder(Folder):
    ...
    def load(self, into_memory=True):
        ...
        if self.name + '_b4.npy' in self.files:
            self._data.b4 = np.load(os.path.join(self.path, self.name + '_b4.npy'), mmap_mode=memory_map_mode, allow_pickle=True)

        self._data.a_cov = np.load(os.path.join(self.path, self.name + '_a_cov.npy'), mmap_mode=memory_map_mode, allow_pickle=True)
        self._data.b_cov = np.load(os.path.join(self.path, self.name + '_b_cov.npy'), mmap_mode=memory_map_mode, allow_pickle=True)
        self._data.C = np.load(os.path.join(self.path, self.name + '_C.npy'), mmap_mode=memory_map_mode, allow_pickle=True)
        self.load_metadata()
