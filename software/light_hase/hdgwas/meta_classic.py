...
class ClassicMetaAnalyser:
    ...
    def __init__(self, meta_phen, meta_pard, sample_intersection, sample_indices, study_names, out,
                 ...
                 t_threshold_full_log=None, flip_path=None):
    ...
    def analyse_genotype_chunk(self, genotype, variant_names, variant_indices,
        ...
        for phenotype, phenotype_names, phenotype_indices in self.meta_phen:
            ...
            for cohort in self.cohort_list:
               ...
               # cohort.set_flip_file(self.flip_path)
...
class CohortAnalyser:
    ...
    def __init__(self, study_index, study_name, row_index,
                 ...
                 maf_threshold=0.0, file_path_cohort=None,
        ...
        self.file_path_cohort = file_path_cohort
        self.swap_map = None

   # def load_flip_file(self, file_path_cohort):
   #     flip_df = pd.read_csv(file_path_cohort, sep="\t", usecols=['variant_index', 'swap'])
   #     self.swap_map = flip_df.set_index(flip_df['variant_index'].astype(np.int64))['swap']
   #     print("Flip file load for {}".format(self.study_name))

    def set_flip_file(self, flip_file_path):
        filename = "{}_data_allele_flip.txt".format(self.study_name)
        file_path_cohort = os.path.join(flip_file_path, filename)
        if os.path.exists(file_path_cohort):
            flip_df = pd.read_csv(file_path_cohort, sep="\t", usecols=['variant_index', 'swap'])
            self.swap_map = flip_df.set_index(flip_df['variant_index'].astype(np.int64))['swap']
            print("Flip file load for {}".format(self.study_name))
        else:
            print("Warning: Flip file not found at {}".format(self.file_path_cohort))
    ...
    def analyse(self, variant_indices, variant_names, genotype, phenotype, meta_pard):
        ...
        if len(self.get_sliced_phenotype_names()) == 0:
            ...
            self.analyser.t_stat = self.fix_flip(tmp)
    ...
    def _analyse(self, genotype, phenotype):
        ...
        tmp = self.complete_output_with_missingness(t_stat[:, 0, :])
        self.analyser.t_stat = self.fix_flip(tmp)
    ...
    def fix_flip(self, tmp):
        # swap_map = flip.set_index(flip['variant_index'].astype(np.int64))['swap']
        rsid_swaps = self.swap_map.reindex(self.analyser.rsid).values
        multiplier = -1 * rsid_swaps
        t_stat_fix = tmp * multiplier[:, np.newaxis]
        return t_stat_fix
